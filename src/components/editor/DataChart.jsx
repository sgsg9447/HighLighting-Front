  ...
  // }, [TIMELINE])
	...
	useEffect(() => {
		...
		const chartList = new Array(CHANNELS).fill(0).map((_, i) => {
			...
			const axisY = chart.getDefaultAxisY().setTitle(`${name}`).setThickness({ min: 50 })
			const axisTime = chart.getDefaultAxisX().setThickness({ min: 30 }).setTickStrategy(AxisTickStrategies.Time)
		...
		Promise.all(
			dataSets.map((data, i) => {
				...
				const yMin = data.reduce((min, cur) => Math.min(min, cur.y), Number.MAX_SAFE_INTEGER)
				const yMax = data.reduce((max, cur) => Math.max(max, cur.y), -Number.MAX_SAFE_INTEGER)
		...
		const resultTableRows = new Array(1).fill(0).map(_ => resultTable.addElement(UIElementBuilders.TextBox))
		resultTable.dispose()
...
// 차트 플레이 바 나타내기 시도
  ...
  return (
    <div>
      ...
      <button onClick={() => {changePointer(prevPointer => prevPointer + 1)}}>
        GET
      </button>
    </div>
  );
